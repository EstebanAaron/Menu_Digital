...
// Nuevo: locales y textos de interfaz por idioma
...
function detectDeviceLang(): Lang {
  ...
  if (typeof navigator !== "undefined" && Array.isArray(navigator.languages)) {
    const match = navigator.languages.map(pick).find((l) => supported.includes(l as Lang));
    if (match) return match as Lang;
  }
  return "es"; // valor por defecto razonable
}

function formatEUR(value: number, locale: string = "es-ES") {
  return new Intl.NumberFormat(locale, { style: "currency", currency: "EUR" }).format(value);
}

// ==== Componentes ====
function DishCard({ dish, lang, isOpen, onToggle }: { dish: Dish; lang: Lang; isOpen: boolean; onToggle: () => void }) {
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <div className="min-w-0">
          <div className="text-lg font-semibold truncate">{dish.name}</div>
          <div className="mt-1 text-sm text-gray-500">{I18N[lang].categories[dish.category]}</div>
        </div>
        <div className="flex items-center gap-3">
          <div className="whitespace-nowrap text-base font-semibold">{formatEUR(dish.price, LOCALE_BY_LANG[lang])}</div>
          <ChevronDown
...
export default function DigitalMenu() {
  ...
  // Opcional: ordenar por categoría y nombre
  const dishesSorted = useMemo(
    () =>
      [...DISHES].sort((a, b) => {
        if (a.category === b.category) return a.name.localeCompare(b.name);
        return Object.keys(CATEGORY_LABELS).indexOf(a.category) -
          Object.keys(CATEGORY_LABELS).indexOf(b.category);
      }),
    []
  );

  // Agrupar por categoría para secciones con títulos
  const grouped = useMemo(() => {
    return dishesSorted.reduce<Record<Category, Dish[]>>((acc, d) => {
